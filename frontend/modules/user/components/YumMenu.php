...
class YumMenu extends CMenu {
	public function init() {
		...
		Yii::app()->clientScript->registerScript('menutoggle_yum', 
				"$(document).on(\"click\", \".user_menu_parent\", function(){ $(this).next().fadeToggle(250); }); ",CClientScript::POS_HEAD);
	...
	protected function normalizeItems($items,$route,&$active) {
		foreach($items as $i=>$item) {
			...
			if(isset($item['items'])) {
				$items[$i]['linkOptions']['class'] .= ' user_menu_parent';
